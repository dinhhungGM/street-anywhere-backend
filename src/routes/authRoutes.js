...
router.post(
  '/sign-in',
  body('username', 'Username does not empty').trim().not().isEmpty(),
  body('password', 'Password does not empty').trim().not().isEmpty(),
...
router.post(
  ...
  body('firstName', 'First name does not empty').trim().not().isEmpty(),
  body('firstName', 'Your first name have to contain at most 50 characters').trim().isLength({ max: 50 }),
  body('lastName', 'Last name does not empty').trim().not().isEmpty(),
